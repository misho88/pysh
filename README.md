...
The aforementioned limitations of `subprocess` primarily had essentially one root cause: all sorts of functionally distinct features are implemented as part of `subprocess.Popen` in a way that makes it difficult to decouple them. As a result, in `pysh`, these are implemented separately and include:
